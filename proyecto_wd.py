...
def entry_text(posicion):
    ...
    root2.geometry("780x100+%d+%d" %(int(posicion[0])+(width/8),int(posicion[1])+(9/2)*height/10))
    entry = Entry(frame, width = 10, bd = 1,textvariable = dynamicvar, font = ('Rockwell Condensed',size),  justify = CENTER)
    entry.focus()
    boton = Button(frame, text = "OK",command = lambda : obtener_dato(entry,root2), width = 10, font = ('Rockwell Condensed',40))
    boton_cancel = Button(frame, text = "VOLVER",command = lambda : obtener_dato(None,root2), width = 10, font = ('Rockwell Condensed',40))
...
def dibuja_operacion(num1, operador, num2):
    ...
    canvas.create_text(width/2, (10/9)*height/4, anchor = CENTER, text = text, font =('Bernard MT Condensed', size)) #tambien cambie el tamano jejejeje-----------------------------
    root.update()
...
def dibuja_pantalla (s, sz, font, cond):
    ...
    canvas.create_image(width/2, height*3/4, image=img)
    canvas.create_text(0, 0, anchor = NW, text = "Puntuación: " + str(puntuacion), font =('Rockwell Condensed' , size))
    canvas.create_text(width/2, (9/2)*height/10, anchor=CENTER, text = s, font = (font, sz))
    root.update()
    time.sleep(3)
...
def cambio_nivel (s, listapuntuacion, rango):
    ...
    Parámetros:
    s -- texto que se grafica al final de terminar todos los niveles de la operación.
    listapuntuacion -- lista que contiene los puntos que se pueden lograr.
    rango -- cambio del rango de la dificultad de las preguntas.
    """
        ...
        if puntuacion == puntos:
            ...
            if len(rango) != 0:
                 ...
                 if puntuacion != listapuntuacion[-1]:
                     ...
                     dibuja_pantalla("Siguiente nivel", size, 'Bernard MT Condensed', True)
                 else:
    ...
    for puntos in listapuntuacion:
        while puntuacion < puntos:
            ...
            while True:
                res1 = pedir_numero()
                if res1 == -1:
                    dibuja_pantalla ("Ingresa un número entero", size, 'Gloucester MT Extra Condensed', None)
                    dibuja_operacion (a, operador, b)
                else:
                    break
            if res1 is None:
            ...
            if res1 == res:
                ...
                if result == result1:
                    ...
                    dibuja_pantalla ("Respuesta incorrecta" + "\n" + "La respuesta correcta es: " + str(result), '55', 'Gloucester MT Extra Condensed', False)
        ...
        while puntuacion < puntos:
            ...
            if a>b:
                ...
                if res1 == res:
                    ...
                    """Si la respuesta es incorrecta, la pregunta vuelve a aparecer"""
                    dibuja_pantalla( "Inténtalo de nuevo" + "\n", size, 'Gloucester MT Extra Condensed', None)
                    num1 = b
                    num2 = a
                    result = b-a
                    dibuja_operacion (num1, operador, num2)
